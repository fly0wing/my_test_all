...
public class HttpClientUtils {
    ...
    private static String get(String url, List<NameValuePair> params) {
        return getWithLoop(url, params, null);
    }

    private static int timeout = 5000;

    public static String getWithLoop(String url, List<NameValuePair> params, Header[] headers) {
        String body = null;
        boolean succFlag = false;
        while (!succFlag) {
            try {
                // Get请求
                body = get(url, params, headers);
                succFlag = true;
            } catch (ParseException | URISyntaxException | IOException ignore) {
            }
        }
        return body;
    }

    private static String get(String url, List<NameValuePair> params, Header[] headers) throws IOException, URISyntaxException {
        String body = null;
...
//            logger.debug("响应:" + body);
    ...
    public static String postWithLoop(String url, List<NameValuePair> params, Header[] asyncHeaders) {
        String body = null;
        boolean succFlag = false;
        while (!succFlag) {
            try {
                body = post(url, params, asyncHeaders);
                succFlag = true;
    ...
    private static String post(String url, List<NameValuePair> params, Header[] asyncHeaders) throws IOException {
        String body = null;
        // Post请求
        HttpPost httppost = new HttpPost(url);
        httppost.getParams().setParameter(CoreConnectionPNames.CONNECTION_TIMEOUT, 5000);//连接时间
        httppost.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT, 5000);//数据传输时间
        // 设置参数
